...
<div
	class="fieldcontain ${hasErrors(bean: userInstance, field: 'hasSkill', 'error')} ">
	<label for="hasSkill"> <g:message code="user.hasSkill.label"
			default="Has Skill" />
	...
	<ul class="one-to-many">
		<g:each in="${userInstance?.hasSkill?}" var="h">
			<li><g:link controller="userSkill" action="show" id="${h.id}">
					${h?.encodeAsHTML()}
				</g:link></li>
		</g:each>
		<li class="add"><g:link controller="userSkill" action="create"
				params="['user.id': userInstance?.id]">
				${message(code: 'default.add.label', args: [message(code: 'userSkill.label', default: 'UserSkill')])}
			</g:link></li>
	</ul>
...
<div
	class="fieldcontain ${hasErrors(bean: userInstance, field: 'memberOf', 'error')} ">
	<label for="memberOf"> <g:message code="user.memberOf.label"
			default="Member Of" />
	...
	<ul class="one-to-many">
		<g:each in="${userInstance?.memberOf?}" var="m">
			<li><g:link controller="memberOfProject" action="show"
					id="${m.id}">
					${m?.encodeAsHTML()}
				</g:link></li>
		</g:each>
		<li class="add"><g:link controller="memberOfProject"
				action="create" params="['user.id': userInstance?.id]">
				${message(code: 'default.add.label', args: [message(code: 'memberOfProject.label', default: 'MemberOfProject')])}
			</g:link></li>
	</ul>
