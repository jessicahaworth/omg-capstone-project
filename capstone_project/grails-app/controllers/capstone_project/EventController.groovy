...
class EventController {
	...
	def list = {
		def (startRange, endRange) = [
			params.long('start'),
			params.long('end')
		].collect { new Instant(it  * 1000L).toDate() }
		...
		withFormat {
			html { [eventInstanceList: eventList] }
			json { render eventList as JSON }
		}
	}
	...
	def show = {
		def (occurrenceStart, occurrenceEnd) = [
			params.long('occurrenceStart'),
			params.long('occurrenceEnd')
		]
